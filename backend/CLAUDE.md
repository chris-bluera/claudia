...
- Session tracking with database as single source of truth
- Tool execution tracking and metrics
- Settings aggregation from Claude Code hierarchy
- File monitoring for session detection
- Loguru logging with file rotation and compression
- Exception-based error handling with custom domain exceptions
...
│   ├── constants.py         # Constants and event types
│   ├── db/
│   │   ├── database.py      # Database connection and session management
│   │   └── models.py        # SQLAlchemy models
│   └── services/
│       ├── file_monitor.py       # Watch Claude Code directories
│       ├── session_tracker.py    # Track sessions in database
│       └── settings_aggregator.py # Aggregate settings hierarchy
├── logs/                    # Log files (gitignored)
│   ├── app.log             # All logs (10MB rotation, 30d retention)
│   └── errors.log          # Errors only (50MB rotation, 60d retention)
├── scripts/
...
# Connect to database
docker exec -it claudia-postgres psql -U claudia -d claudia
